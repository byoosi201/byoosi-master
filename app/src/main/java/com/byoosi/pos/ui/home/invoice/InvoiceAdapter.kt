...
class InvoiceAdapter : BaseAdapter<InvoiceItem>(R.layout.listitem_invoice) {
    ...
    override fun onBind(view: View, position: Int, item: InvoiceItem, payloads: MutableList<Any>) {
        view.run {
            ...
            tvStatus.text = when {
                (item.outstanding_amount).toInt() != 0 -> context.getString(R.string.unpaid)
                item.outstanding_amount.toInt() == 0 -> context.getString(R.string.paid)
                else -> context.getString(R.string.partially_paid)
            }
        }
    }
}